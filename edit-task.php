...
<body>
    ...
    <!-- Existing input fields for title, description, assigned_to, due_date -->
					<div class="input-holder">
						...
						<select name="priority" class="input-1">
							...
							<option value="Medium" <?= isset($task['priority']) && $task['priority'] === 'Medium' ? 'selected' : 'selected' ?>>Medium</option>
							<option value="Low" <?= isset($task['priority']) && $task['priority'] === 'Low' ? 'selected' : '' ?>>Low</option>
						</select><br>
					</div>
				<!-- willr eturn to this partner assign in the future -->
            <div class="input-holder"> 
					<lable>Assigned to</lable>
					<select name="assigned_to" class="input-1">
						<option value="0">Select Friend To Work With</option>
						<?php if ($users !=0) { 
